...
CollapseCard(
  ...
  template(#action="{ item }")
    .card-action(v-if="item.schedule == '訂單已成立'")
      .alert
        .notice
          .icon
          ...
          .text
            ...
            br 
            b 小時可點按預約開始
        .right-btn(@click="onStart(item.orderId)") 開始
    .card-action(v-if="item.schedule == '訂單進行中'")
      .alert
        .notice
          .icon
            svg(xmlns='http://www.w3.org/2000/svg' width='24' height='24' viewBox='0 0 24 24' fill='none')
          ...
          .text
            ...
            br 
            b 訂單即完成
        .right-btn(@click="onEnd(item.orderId)") 結束

</template>

<script lang="ts">
import { defineComponent, ref, computed } from 'vue';
import { showSuccessToast, showConfirmDialog, showLoadingToast, showToast} from "vant";
import { postNurseBookingEmpty, postNurseBookingStart, postNurseBookingEnd} from "@/api/nurse/booking";
import { IPostNurseBookingEmpty } from "@/Interface";
    ...
    const receive = (orderId: string) => {
      showConfirmDialog({
        ...
        .then(async() => {
          ...
          let params: IPostNurseBookingEmpty ={
            userLineId: profile.value._CLIENT_PROFILE_KEY,
            orderId: orderId
          }
          await postNurseBookingEmpty(params)
          .then(() => {
            loading.close();
          })
          .finally(() => {
    ...
    const onStart = (orderId: string) => {
      showConfirmDialog({
        ...
        .then(async() => {
          ...
          await postNurseBookingStart(params)
          .then(() => {
            loading.close();
          })
          .finally(() => {
    ...
    const onEnd = (orderId: string) => {
      showConfirmDialog({
        ...
        .then(async() => {
          ...
          await postNurseBookingEnd(params)
          .then(() => {
            loading.close();
          })
          .finally(() => {
